...
st.write("This program reads a CSV file with TV program ratings and generates an optimal schedule.")

# File Path
file_path = "program_ratings.csv"  # Replace with your file's name or path
...
try:
    ...
    st.write("Loaded Ratings Data:")
    st.json(program_ratings_dict)
except FileNotFoundError:
    st.error(f"File '{file_path}' not found. Please ensure it is in the correct location.")
    st.stop()
...
def mutate(schedule):
    ...
    schedule[idx1], idx2 = schedule[idx2], schedule[idx1]
    return schedule
